...
export class Arguments {
   ...
   */
  private static parseNumber<T extends number>(arg: string, defaultValue: T): T {
  ...
  getNumberRange<T extends number>(defaultStart: T, defaultEnd: T): [T, T] {
    switch (this.type) {
      case ArgumentsType.None:
        return [defaultStart, defaultEnd];
      case ArgumentsType.Fixed:
        return [Arguments.parseNumber(this.value || "", defaultStart), defaultEnd];
      case ArgumentsType.Range:
        return [
  ...
  getDateTimeRange(defaultStart: Date, defaultEnd: Date): [Date, Date] {
    switch (this.type) {
      case ArgumentsType.None:
        return [defaultStart, defaultEnd];
      case ArgumentsType.Fixed:
        return [Arguments.parseDateTime(this.value || "", defaultStart), defaultEnd];
      case ArgumentsType.Range:
        return [
